...
def download_matrix(intent_name):
    ...
    client.fget_object(bucket_name, "ChatBotData/" + intent_name, filename)
    # f.close()
...
def load_bow():
    ...
    client.fget_object(bucket_name, "ChatBotData/" + 'bos.txt', filename)
    # f.close()
    BOW = []
...
def mainFunc(event):
    ...
    pre_process_duration = int(event['duration'])
    BOW = load_bow()
    ...
    for w in range(num_workers):
        ...
        skew = int(list_of_intents[w]["skew"])
        network_bound = int(list_of_intents[w]['network_bound'])
        ths.append(
...
def load_intents(filename):
    client.fget_object(bucket_name, "ChatBotData/Clever_neg.txt", "/tmp/Clever_neg.txt")
