...
def run_workflows(namespace, workflow_name, filename, times, inp, log_file_name):
    results = {}
    for i in range(int(times)):
        #run_name, result = run_workflow(namespace, filename)
        run_name = workflow_name + "-" + create_random_name(5)
        results[run_name] = run_video(inp)
        with open("logs2/" + log_file_name, 'a') as file:
            file.write(run_name + " " + results[run_name] + "\n")
        time.sleep(30)
    return results
    
def run(workflow_name, function_index, conc_index, cpu_index, timeout_index, times, inp):
    ...
    result = run_workflows('argo-wf', workflow_name, filename, times, inp, log_file_name)
    final = log_file_name[:-4] + " " +  analyze_result(list(result.values()), 1)
    return final

def main():
    inp = {"src_name": "0", "DOP": "30", "detect_prob": 2}
    pprint(run("video", 3, 1, 3, '-', 10, inp))
    pprint(run("video", 3, 2, 3, '-', 10, inp))
    pprint(run("video", 3, 1, 2, '-', 10, inp))
    pprint(run("video", 3, 2, 2, '-', 10, inp))
    pprint(run("video", 3, 1, 1, '-', 10, inp))
    pprint(run("video", 3, 2, 2, '-', 10, inp))
    pprint(run("video", 3, 1, 0, '-', 10, inp))
    pprint(run("video", 3, 2, 0, '-', 10, inp))
